...
var types = 'array boolean date element empty finite function null \
            number object regexp string'.match(/(\w+)/g),
    typeConstraints = {
    ...
    _isPlainObject = typeConstraints.isPlainObject,
    ...
    _normalizeValidationSpec = function(spec) {
        return _.reduce(spec, function(normalized, value, key) {
...
var Validate = {
     ...
     * @param {array} spec Validation spec (array of objects) to validate against
     ...
     * @param {object} spec Validation spec ( key => validation ) to validate against
     * @return {validationObject} A validation object telling you what happened
     */
    ...
    validate: function(values, spec) {
        ...
        if (typeConstraints.isPrimitive(spec)) spec = [spec];
        ...
        return validationObject(errors, values, _normalizeValidationSpec(spec));
    },

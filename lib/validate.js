...
var types = 'array boolean date element empty finite function null \
    ...
    validationObject = function(invalidKeys, values, spec) {
        return {
            isValid: function() {
            ...
            errorString: function() {
                ...
                var errors = _.map(invalidKeys, function(key) {
                    ...
                    return prefix + ' argument ' + key + ' is not a "' + spec[key].isa + '"';
                });
...
var Validate = {
    ...
    validate: function(values, spec) {
        ...
        if (typeConstraints.isPrimitive(spec)) return Validate.positional(values, [spec]);
